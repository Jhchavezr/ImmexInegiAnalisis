...
## Análisis inicial de la información Agrupada por Municipios
...
## Ejemplo de Microdatos (a consultar via INEGI)

```{r read_microdatos, echo=FALSE}
# path to example microdata
csv_path <- "baseDeDatos/Microdatos/EjemploManufacturas.csv"
...
# read safely
dfExample <- readr::read_csv(csv_path, show_col_types = FALSE)

# show dimension
cat("Dimensions (rows, cols):", paste(dim(dfExample), collapse = " x "), "\n\n")

# show first 5 rows and first 5 variables (handle if fewer than 5 cols)
ncols_show <- min(5, ncol(dfExample))
print(dfExample[1: min(5, nrow(dfExample)), 1:ncols_show])

# show available variables
cat("\nAvailable variables (column names):\n")
print(names(dfExample))
```
